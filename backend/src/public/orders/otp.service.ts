...
export class PublicOtpService {
    ...
    async sendOtp(phone: string, merchantId: string) {
        ...
        if (!user || user.role !== Role.RESELLER) {
            throw new BadRequestException('Hanya Reseller yang membutuhkan verifikasi OTP untuk harga khusus.');
        }
    ...
    async verifyOtp(phone: string, merchantId: string, code: string) {
        ...
        if (!stored) throw new BadRequestException('OTP tidak ditemukan atau sudah kadaluarsa.');
    ...
    isVerified(phone: string, merchantId: string, token: string): boolean {
        ...
        if (!stored || stored.code !== token || stored.expires < Date.now()) return false;
        return true;
    }
}
