...
export class TripayService {
    ...
    private async getConfigs(merchantId?: string) {
        ...
        const keyMap = settings.reduce((acc, s) => ({ ...acc, [s.key]: s.value }), {} as any);
     ...
     */
    async verifySignature(callbackSignature: string, payload: string | Buffer, merchantId?: string): Promise<boolean> {
