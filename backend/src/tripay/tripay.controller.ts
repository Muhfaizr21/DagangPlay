...
interface TripayWebhookPayload {
  ...
  fee_customer: number;
  total_amount: number;
  status: 'PAID' | 'EXPIRED' | 'FAILED' | 'UNPAID';
...
export class TripayController {
  ...
  async tripayCallback(
    ...
    try {
      ...
      const configAllowedIps = process.env.TRIPAY_ALLOWED_IPS
        ? process.env.TRIPAY_ALLOWED_IPS.split(',')
        : ['95.111.200.230', '127.0.0.1'];
      ...
      const isAllowed =
        configAllowedIps.some((ip) => clientIp.includes(ip.trim())) ||
      ...
      if (!isAllowed) {
        ...
        return res
          ...
          .json({ success: false, message: 'Unauthorized IP Source' });
      }

      // Deferred Signature Verification
      const data = req.body as TripayWebhookPayload;
      ...
      if (!ref) {
        ...
        return res
          ...
          .json({ success: false, message: 'Missing merchant_ref' });
      }
      ...
      const isValid = await this.tripayService.verifySignature(
        signature,
        rawBody,
        merchantId,
      );
      ...
      if (!isValid) {
        console.warn(
          `[TripayCallback] Invalid signature for Ref: ${ref} (Merchant: ${merchantId || 'PLATFORM'}). Verification failed.`,
        );
        return res
          .status(HttpStatus.FORBIDDEN)
      ...
      console.log(
        `[TripayCallback] Received ${data.status} for Ref: ${ref} (Merchant: ${merchantId || 'PLATFORM'})`,
      );
        ...
        if (ref.startsWith('ORD-')) {
          try {
            ...
            if (order.paymentStatus !== 'PENDING') {
              console.log(
                `[TripayCallback] Order ${ref} already processed (${order.paymentStatus}). Skipping.`,
              );
              ...
              if (netProfit > 0) {
                ...
                if (merchant) {
                  ...
                  if (
                    ...
                    'platformFee' in merchant.settings
                  ) {
                    platformFeePct = Number(merchant.settings.platformFee) || 0;
                  }

                  const platformFeeAmount = Math.round(
                    netProfit * (platformFeePct / 100),
                  );
                  ...
                  if (merchantNetProfit > 0) {
                    ...
                    await tx.merchantLedgerMovement.create({
                      data: {
                        ...
                        escrowBefore:
                          updatedMerchantEscrow.escrowBalance -
                          merchantNetProfit,
                  ...
                  const saasMarkup = Math.max(
                    0,
                    Number(order.merchantModalPrice || 0) -
            ...
            this.whatsappService
              ...
              .catch((err) =>
                console.error(
                  `[TripayCallback] Failed notification:`,
                  err.message,
                ),
              );
            ...
            console.log(
              `[TripayQueue / Persisted] Enqueued fulfillment specifically for order: ${order.orderNumber}`,
            );
          } catch (err: any) {
            if (err.code === 'P2025') {
              console.log(
                `[TripayCallback] Race condition prevented for order ${ref}. Already processed.`,
              );
              return res.status(HttpStatus.OK).json({ success: true });
            }
            throw err;
        ...
        // 2. Check if it's a DEPOSIT (Ref: DEP-...)
          ...
          try {
            ...
            if (deposit && deposit.status === 'PENDING') {
              ...
              const tripayFeeMerchant = Number(data.fee_merchant) || 0;
              const netDepositAmount = Math.max(
                0,
                Number(data.amount) - tripayFeeMerchant,
              );
              ...
              console.log(
                `[TripayCallback] Deposit ${depositId} confirmed for Merchant: ${deposit.merchantId}. Net: ${netDepositAmount}`,
              );
            }
          } catch (err: any) {
            if (err.code === 'P2025')
            ...
            if (
              ...
              await this.prisma.$transaction(async (tx) => {
                ...
                if (updatedInvoice.count === 0) {
                  console.log(
                    `[TripayCallback] Race condition detected for invoice ${ref}. Already processed.`,
                  );
                  return;
                }
                ...
                const now = new Date();
                const currentExpiry =
                  merchant?.planExpiredAt && merchant.planExpiredAt > now
              ...
              console.log(
                `[TripayCallback] Invoice ${ref} paid. Merchant plan upgraded.`,
              );
            }
          } catch (err: any) {
            if (err.code === 'P2025')
        ...
        if (ref.startsWith('ORD-')) {
          try {
            ...
            if (order && order.paymentStatus === 'PENDING') {
              ...
              console.log(
                `[TripayCallback] Order ${ref} updated to ${data.status}.`,
              );
            }
          } catch (err: any) {
            if (err.code === 'P2025')
          ...
          try {
            ...
            if (deposit && deposit.status === 'PENDING') {
              ...
              console.log(
                `[TripayCallback] Deposit ${depositId} updated to ${data.status}.`,
              );
            }
          } catch (err: any) {
            if (err.code === 'P2025')
          ...
          try {
            ...
            if (
              ...
              console.log(
                `[TripayCallback] Invoice ${ref} updated to ${data.status}.`,
              );
            }
          } catch (err: any) {
            if (err.code === 'P2025')
