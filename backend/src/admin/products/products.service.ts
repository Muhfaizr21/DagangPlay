...
export class ProductsService {
  ...
  async getCategories() {
    ...
    return categories.map((cat) => ({
      ...cat,
      totalSkus: cat.products.reduce(
        (acc, p) => acc + (p._count?.skus || 0),
        0,
      ),
    }));
  }
  ...
  async syncDigiflazzProducts() {
    try {
      ...
      if (!response.ok || !jsonResp.data || !Array.isArray(jsonResp.data)) {
        ...
        throw new Error(
          `Digiflazz Sync Error: ${JSON.stringify(jsonResp.data || jsonResp)}`,
        );
      }
      ...
      allRules
        .filter((r) => r.categoryId && r.isActive)
        .forEach((r) => rulesByCategory.set(r.categoryId, r));
      ...
      for (const item of items) {
        ...
        const thumbnail =
          ...
          `https://www.google.com/s2/favicons?sz=128&domain=${brandSlug}.com`;
      ...
      return {
        success: true,
        message: `Sinkronisasi Berhasil: ${newCount} baru, ${updatedCount} diupdate.`,
        newCount,
        updatedCount,
      };
    } catch (err: any) {
      ...
      throw new InternalServerErrorException(
        err.message || 'Gagal sinkronisasi produk',
      );
    }
  }
  ...
  async applyCategoryFormula(categoryId: string, margins: any) {
    const where =
      categoryId === 'global' || categoryId === 'all'
        ? {}
        : { product: { categoryId } };
  ...
  async getPublicCategories(merchantSlug?: string, domain?: string) {
    ...
    for (const cat of categories) {
      ...
      if (lowSlug.startsWith('free-fire')) {
        ...
        lowSlug === 'mlbb' ||
        lowSlug === 'mobile-legend' ||
        lowSlug === 'mobile-legends'
      ) {
    ...
    // Urutan Produk: Sort by sortOrder DESC, then name ASC
  ...
  async getPublicCategoryBySlug(
    ...
    const merchantPrices = merchantId
      ? await this.prisma.merchantProductPrice.findMany({
          where: { merchantId },
        })
      : [];

    const priceMap = new Map(
      merchantPrices.map((mp) => [mp.productSkuId, mp.customPrice]),
    );
    const visibilityMap = new Map(
      merchantPrices.map((mp) => [mp.productSkuId, mp.isActive]),
    );

    // Merge results into a single object
    const primary = categories[0];
    const allProducts = (categories as any)
      .flatMap((c) =>
        c.products.map((p) => {
          const filteredSkus = p.skus
            .filter((s) => {
            ...
            .map((s) => {
              ...
              return {
                ...s,
                priceNormal:
                  customPrice !== undefined ? customPrice : s.priceNormal,
              };
            });
    ...
    if (
      canonicalName.includes('mobile legend') ||
      canonicalName.includes('mlbb')
    ) {
      ...
      canonicalName.includes('genshin') ||
      canonicalName.includes('honkai')
    ) {
      mergedResult.products = mergedResult.products.map((p) => ({
        ...p,
        gameIdLabel: 'UID',
  ...
  async getPublicFullCatalog(merchantSlug?: string, domain?: string) {
    const merchant: any = await this.resolveMerchant(merchantSlug, domain);
      ...
      .map((cat) => {
        const products = cat.products
          .map((p) => {
            ...
            const imageFallback =
              fallbacks[p.slug] || fallbacks[cat.slug] || cat.image;

            const override = p.overrides?.[0];
            const finalName = override?.customName || p.name;
            const finalThumbnail =
              override?.customThumbnail || p.thumbnail || imageFallback;
