...
export class MarketingService {
  ...
  async getGuideById(id: string) {
    const guide = await this.prisma.marketingGuide.findUnique({
      where: { id },
    });
  ...
  async getGuidesForMerchant(merchantId: string) {
    ...
    if (setting) {
      ...
      const merchantPlanFeatures =
        planFeatures[merchant.plan as string] || planFeatures['FREE'] || {};
    ...
    return this.prisma.marketingGuide.findMany({
      where: {
        ...
        OR: [
          { targetPlan: 'FREE' },
          ...(currentWeight >= 1
            ? [{ targetPlan: 'PRO' as MerchantPlan }]
            : []),
          ...(currentWeight >= 2
            ? [{ targetPlan: 'LEGEND' as MerchantPlan }]
            : []),
          ...(currentWeight >= 3
            ? [{ targetPlan: 'SUPREME' as MerchantPlan }]
            : []),
        ],
      },
      orderBy: { sortOrder: 'asc' },
