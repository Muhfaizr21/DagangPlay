...
export class CommissionsService {
  ...
  async settleCommission(id: string, operatorId: string) {
    ...
    if (commission.status !== 'PENDING')
      throw new BadRequestException('Bukan status PENDING');
  ...
  async autoSettleCommissions() {
    this.logger.log(
      '[Cashflow Protect] Menjalankan settlement otomatis untuk komisi PENDING >24 jam...',
    );
    ...
    if (settledCount > 0) {
      this.logger.log(
        `[Cashflow Protect] Berhasil mencairkan ${settledCount} komisi secara otomatis.`,
      );
    }
  }
