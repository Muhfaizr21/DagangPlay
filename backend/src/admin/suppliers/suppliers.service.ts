...
export class SuppliersService {
    ...
    async updateSupplier(id: string, data: { name?: string; apiUrl?: string; apiKey?: string; apiSecret?: string; status?: SupplierStatus }) {
    ...
    async testConnection(id: string) {
        ...
        if (supplier.code === 'DIGIFLAZZ') {
            try {
                ...
                if (!username || !key) throw new Error('Digiflazz Credentials not configured in .env');
            ...
            throw new InternalServerErrorException('Ping test belum didukung untuk supplier ini');
        }
    }
