    ...
    async getDashboardSummary(range: string = 'WEEK') {
        ...
        const chartData = [...Array(intervals)].map((_, i) => {
            ...
            if (isMonthly) {
                ...
                return { label, value: Math.round(revenue / 1000000), profit: Math.round(profit / 1000000) }; // JT for Year
            } else {
                ...
                return { label, value: Math.round(revenue / 1000), profit: Math.round(profit / 1000) }; // RB for Month/Week
            }
        });
