...
export class DigiflazzService {
  ...
  private getDigiflazzConfig() {
    ...
    if (!username || !key) {
      throw new InternalServerErrorException(
        'Kredensial Digiflazz tidak dikonfigurasi pada environment variables (.env)',
      );
    }
  ...
  async getDigiflazzProducts() {
    try {
      ...
      if (this.priceListCache && now - this.lastFetchTime < 60000) {
        console.log(
          '[DigiflazzService] Menggunakan data cache Digiflazz (Menghindari limitasi rate)',
        );
        ...
        if (response.status !== 200 || !jsonResp.data) {
          throw new InternalServerErrorException(
            `Digiflazz Error: ${JSON.stringify(jsonResp)}`,
          );
        }
        ...
        if (!Array.isArray(jsonResp.data)) {
          console.warn(
            '[DigiflazzService] API Digiflazz Error/Limit.',
            jsonResp.data,
          );
          ...
          if (fs.existsSync(cachePath)) {
            console.log(
              '[DigiflazzService] Memuat data Digiflazz dari local disk cache...',
            );
            rawItems = JSON.parse(fs.readFileSync(cachePath, 'utf8'));
          } else {
            // Jika tidak ada cache, biarkan kosong agar tidak membingungkan operator dengan data "palsu"
            console.warn(
              '[DigiflazzService] Tidak ada local file cache. Produk tidak dapat dimuat.',
            );
            rawItems = [];
          }
        } else {
          ...
          console.log(
            '[DigiflazzService] Data Digiflazz asli berhasil disimpan ke local disk cache.',
          );
        }
      }
      ...
      throw new InternalServerErrorException(
        `Digiflazz Connection Error: ${msg}`,
      );
    }
  }
    ...
    try {
      ...
      if (existingSku) {
        ...
        savedSku = await this.prisma.productSku.update({
          ...
          data: {
            ...
            marginLegend:
              (dto.priceLegend || dto.priceNormal) - dto.digiflazz_price,
            marginSupreme:
              (dto.priceSupreme || dto.priceNormal) - dto.digiflazz_price,
        ...
        if (
          Number(existingSku.priceNormal) !== Number(dto.priceNormal) ||
          Number(existingSku.basePrice) !== Number(dto.digiflazz_price)
        ) {
          await this.prisma.auditLog.create({
            data: {
              ...
              oldData: {
                priceNormal: Number(existingSku.priceNormal),
                basePrice: Number(existingSku.basePrice),
              },
            },
          });
        }
      ...
      for (const mp of merchantPrices) {
        ...
        if (currentModalPrice > mp.customPrice) {
          ...
          console.log(
            `[NegativeMarginAlert] Product ${savedSku.supplierCode} deactivated for Merchant ${mp.merchant.name}`,
          );
        }
      }
      ...
      return {
        success: true,
        message: 'Produk berhasil di-sync dan di-mapping',
        sku: savedSku,
      };
    } catch (err: any) {
      console.error('[DigiflazzService] Error syncing product:', err);
      throw new InternalServerErrorException(
        'Gagal melakukan sinkronisasi produk (' + err.message + ')',
      );
    }
  }
    ...
    setImmediate(async () => {
      ...
      for (const p of payload) {
        try {
          ...
          console.error(
            `[BulkSync] Gagal sync produk ${p.buyer_sku_code}:`,
            err,
          );
        }
      }
      console.log(
        `[BulkSync] Selesai: ${successCount} dari ${payload.length} produk tersinkronisasi.`,
      );
    });

    return {
      success: true,
      message: `Proses sinkronisasi massal untuk ${payload.length} produk sedang berjalan di latar belakang.`,
    };
  }
  ...
  async checkOrderStatus(
    ...
    const sign = crypto
      .createHash('md5')
      .update(username + key + supplierRefId)
      .digest('hex');
  ...
  async checkBalance() {
    ...
    if (
      ...
      key === 'your_digiflazz_key'
    ) {
      this.logger.warn(
        '[DigiflazzService] INFO: Menggunakan kredensial dummy di .env, mengembalikan mock saldo 0.',
      );
    ...
    try {
      ...
      if (error.response && error.response.data) {
        throw new Error(
          `Digiflazz API Error: ${JSON.stringify(error.response.data)}`,
        );
      }
      throw error;
    }
  ...
  async checkAvailability(skuCode: string) {
    ...
    if (!item) {
      return {
        isAvailable: false,
        reason: 'Produk tidak ditemukan di supplier',
      };
    }
    ...
    if (
      ...
      return {
        ...
        reason:
          (item as any).message ||
          'Produk sedang gangguan atau stok kosong di supplier',
      };
    }
  ...
  async placeOrder(orderId: string) {
    ...
    if (!order.gameUserId || order.gameUserId.trim() === '') {
      this.logger.error(
        `[placeOrder] Order ${order.orderNumber} has no gameUserId. Cannot fulfill.`,
      );
    ...
    try {
      ...
      if (currentBalance < order.basePrice) {
        this.logger.error(
          `[SupplierBalance] Insufficient balance for ${order.orderNumber}. Have: ${currentBalance}, Need: ${order.basePrice}`,
        );
        ...
        await this.prisma.order.update({
          ...
          data: {
            fulfillmentStatus: 'FAILED',
            failReason:
              'Gagal diproses: Saldo supplier sedang tidak mencukupi (Refund Otomatis).',
          },
        });
      ...
      this.logger.error(
        `[SupplierBalance] Failed to verify balance, proceeding normally: ${balErr.message}`,
      );
    }

    // --- ANTI-LOSS GUARD (CRITICAL FIX) ---
    const currentBasePrice = Number(order.productSku.basePrice);
    const lockedModalPrice = Number(
      order.merchantModalPrice || order.sellingPrice,
    );

    // If Digiflazz price jumped above what we charged the merchant, abort and dispute.
    if (currentBasePrice > lockedModalPrice) {
      console.warn(
        `[AntiLoss] Price Spike Detected: Order ${order.orderNumber}. Current Base: ${currentBasePrice} > Locked Modal: ${lockedModalPrice}. ABORTING.`,
      );
    ...
    const sign = crypto
      .createHash('md5')
      .update(username + key + order.orderNumber)
      .digest('hex');

    // Prepare customer number (standard format for ML/FF etc)
    const customerNo = order.gameUserServerId
      ? `${order.gameUserId}${order.gameUserServerId}`
      : order.gameUserId;
    ...
    try {
      ...
      if (resJson.data) {
        ...
        if (fulfillmentStatus === 'SUCCESS' || fulfillmentStatus === 'FAILED') {
          this.whatsappService
            ...
            .catch((err) =>
              this.logger.error(
                `[FulfillmentNotification] Failed: ${err.message}`,
              ),
            );
        ...
        await this.prisma.order.update({
          ...
          data: {
            fulfillmentStatus: 'FAILED',
            failReason:
              resJson.data?.message || 'Digiflazz API connection error',
          },
        });
      ...
      console.error(
        '[DigiflazzService] Fulfillment Connection Error (Potentially Stuck):',
        err.message,
      );
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const commissions = await tx.commission.findMany({
        where: {
          orderId,
          status: 'SETTLED',
          settledAt: { gte: new Date(Date.now() - 5000) },
        }, // newly settled
  ...
  public async handleCustomerRefund(orderId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (isGuest) {
        ...
        console.log(
          `[CustomerRefund] Order ${order.orderNumber} is a GUEST order. Generating Refund Voucher: ${voucherCode}`,
        );
      ...
      console.log(
        `[CustomerRefund] Order ${order.orderNumber} fully refunded to user ${order.user.name}.`,
      );
    });
  }
      ...
      for (const item of payload) {
        ...
        for (const sku of skus) {
          ...
          if (newPrice > Number(sku.priceNormal)) {
            console.error(
              `[AntiLoss] Price Spike! New Base (${newPrice}) > Current Retail (${sku.priceNormal}). DEACTIVATING ${skuCode}.`,
            );
          ...
          await this.prisma.$transaction(async (tx) => {
            ...
            for (const mp of merchantPrices) {
              ...
              const markupSetting = merchantSettings.find(
                (s) => s.key === 'MARKUP_PERCENTAGE',
              );
              ...
              if (markupPercentage > 0) {
                ...
                const newDynamicPrice = Math.ceil(
                  currentModalPrice +
                    currentModalPrice * (markupPercentage / 100),
                );
                console.log(
                  `[DynamicMarkup] Updating Price for Merchant ${mp.merchant.name}: ${mp.customPrice} -> ${newDynamicPrice} (${markupPercentage}%)`,
                );
  ...
  async processTransactionWebhook(data: any) {
    ...
    console.log(
      `[DigiflazzWebhook] Transaksi ${data.ref_id} diupdate ke status ${newStatus}`,
    );
  }
  ...
  async retryStuckOrders() {
    ...
    if (stuckOrders.length > 0) {
      this.logger.log(
        `[Cron] Found ${stuckOrders.length} stuck PROCESSING orders. Retrying...`,
      );
    }
    ...
    for (const order of stuckOrders) {
      try {
        ...
        this.logger.log(
          `[Cron] Successfully hit retry for ${order.orderNumber}`,
        );
      } catch (err: any) {
        this.logger.error(
          `[Cron] Failed to retry order ${order.orderNumber}: ${err.message}`,
        );
      }
    }
  }
   ...
   */
  async bulkAdjustMargins(
    amount: number,
    type: 'PRO' | 'LEGEND' | 'SUPREME' | 'ALL',
  ) {
    this.logger.log(
      `[BulkAdjust] Starting margin adjustment of Rp ${amount} for tier: ${type}`,
    );
