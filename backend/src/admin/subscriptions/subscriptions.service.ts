...
export class SubscriptionsService {
    ...
    async confirmInvoice(id: string, operator: string) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            await tx.merchant.update({
                ...
                data: {
                    plan: targetPlan as MerchantPlan,
    ...
    async checkFeatureLimit(merchantId: string, feature: 'maxProducts' | 'maxMembers' | 'multiUser' | 'whiteLabel' | 'customDomain' | 'flashSale' | 'templateVariants' | 'instantWithdrawal' | 'customProductDetail' | 'buildApk' | 'prioritySupport' | 'resellerAcademy' | 'tldDomain', addingCount: number = 0) {
        ...
        if (feature === 'maxMembers') {
            ...
            if (features.maxMembers !== undefined && totalAfterAdd > features.maxMembers) {
                throw new BadRequestException(`Limit member terlampaui. Paket Anda hanya mengizinkan ${features.maxMembers} member. (Saat ini ${count}/${features.maxMembers})`);
            }
        }
