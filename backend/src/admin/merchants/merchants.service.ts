...
export class MerchantsService {
  ...
  async getAllMerchants(
    ...
    const mappedData = paginated.data.map((m: any) => {
      ...
      const omset = Number(
        omsetAggs.find((oa) => oa.merchantId === m.id)?._sum.totalPrice || 0,
      );
  ...
  async getMerchantDetail(id: string) {
    const merchant = await this.prisma.merchant.findUnique({
      ...
      include: {
        ...
        _count: {
          select: { orders: true, deposits: true, supportTickets: true },
        },
      },
    });
    if (!merchant) throw new NotFoundException('Merchant tidak ditemukan');
  ...
  async updateMerchantSettings(id: string, updateData: any) {
    ...
    const { domain, plan, planExpiredAt, isOfficial, status, ...settingsOnly } =
      updateData;

    // Ensure domain uniqueness if provided
    if (domain && domain !== merchant.domain) {
      const existingDomain = await this.prisma.merchant.findUnique({
        where: { domain },
      });
    ...
    const currentSettings =
      typeof merchant.settings === 'object' && merchant.settings !== null
        ? merchant.settings
        : {};
  ...
  async resetOwnerPassword(merchantId: string) {
    ...
    if (!merchant || !merchant.owner)
      throw new NotFoundException('Merchant/Owner tidak ditemukan');
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        entity: 'Merchant',
        entityId: merchantId,
        newData: { target: merchant.owner.email },
        oldData: {},
      },
    });
    ...
    return {
      success: true,
      message: `Password Owner berhasil direset secara aman. Password baru: ${randomPass}`,
    };
  }
  ...
  async exportMerchantsCsv() {
    ...
    let csv =
      'Merchant ID,Nama Merchant,Domain,Plan,Status,Owner,Email Owner,Total Trx PAID,Tgl Daftar\n';
