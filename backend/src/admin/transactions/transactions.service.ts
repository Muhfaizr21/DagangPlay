...
export class TransactionsService {
  ...
  async retryTransaction(id: string, operatorId: string) {
    ...
    if (order.fulfillmentStatus === 'SUCCESS')
      throw new BadRequestException('Transaksi sudah sukses');
    ...
    return {
      success: true,
      message: 'Transaksi dimasukkan antrian BullMQ untuk diproses ulang',
    };
  }
  ...
  async refundTransaction(id: string, operatorId: string) {
    ...
    if (order.fulfillmentStatus === 'REFUNDED')
      throw new BadRequestException('Trx sudah direfund');
    if (order.paymentStatus !== 'PAID')
      throw new BadRequestException('Trx belum dibayar, tidak bisa refund');
    // FIX B: Prevent refunding an order that was already fulfilled — prevents double benefit
    if (order.fulfillmentStatus === 'SUCCESS')
      throw new BadRequestException(
        'Tidak bisa refund: Produk sudah berhasil dikirim ke pelanggan. Hubungi supplier terlebih dahulu untuk pembatalan.',
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return {
        success: true,
        message: 'Refund berhasil diproses & Profit merchant ditarik balik',
      };
    });
  }
  ...
  async overrideStatus(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        (fulfillmentStatus === 'FAILED' || fulfillmentStatus === 'REFUNDED')
      ) {
        await this.publicOrders.reverseCommission(id, tx);
      }
