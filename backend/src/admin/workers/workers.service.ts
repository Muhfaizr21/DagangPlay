...
export class WorkersService {
  ...
  async handleJobQueue() {
    this.logger.debug(
      'Menjalankan background jobs Worker dari JobQueue Prisma...',
    );
  ...
  async syncFulfillmentStatus() {
    ...
    for (const order of ordersToSync) {
      try {
        ...
        if (
          order.fulfillmentStatus === OrderFulfillmentStatus.PROCESSING &&
          order.updatedAt < tenMinutesAgo
        ) {
          this.logger.warn(
            `Order ${order.orderNumber} is stuck in PROCESSING for over 10 minutes. Manual check recommended.`,
          );
        ...
        const newStatus =
          statusMap[supplierInfo.status] || order.fulfillmentStatus;
        ...
        if (newStatus !== order.fulfillmentStatus) {
          await this.prisma.order.update({
            ...
            data: {
              ...
              completedAt:
                newStatus === OrderFulfillmentStatus.SUCCESS
                  ? new Date()
                  : null,
              failedAt:
                newStatus === OrderFulfillmentStatus.FAILED ? new Date() : null,
              failReason:
                newStatus === OrderFulfillmentStatus.FAILED
                  ? supplierInfo.message
                  : null,
            },
          });
          ...
          if (newStatus === OrderFulfillmentStatus.FAILED) {
            this.logger.warn(
              `Order ${order.orderNumber} FAILED via Sync. Triggering refund & reversal.`,
            );
            ...
            if (pendingCommission) {
              ...
              this.logger.warn(
                `[SyncCron] Order ${order.orderNumber} FAILED but commissions already reversed. Skipping double-refund.`,
              );
            }
          }
        ...
        this.logger.error(
          `Failed to sync order ${order.orderNumber}: ${err.message}`,
        );
      }
    }
  }
  ...
  async syncProductsFromSupplier() {
    this.logger.log(
      'Starting Automated Product & Price Sync from Digiflazz...',
    );
    try {
      ...
      const gameItems = products.filter((item: any) => {
        ...
        const isTelcoBrand =
          brand.includes('TELKOMSEL') ||
          brand.includes('XL') ||
          brand.includes('AXIS') ||
          brand.includes('INDOSAT') ||
          brand.includes('TRI') ||
          brand.includes('SMARTFREN');
        const isShopping =
          brand.includes('ALFAMART') ||
          brand.includes('INDOMARET') ||
          brand.includes('GRAB') ||
          brand.includes('GOJEK') ||
          brand.includes('PLN');

        return (
          (isGameCategory || type === 'GAMES') && !isTelcoBrand && !isShopping
        );
      });
      ...
      const gameThumbnails: Record<string, string> = {
        'MOBILE LEGENDS':
          'https://cdn1.codashop.com/S/content/common/images/mno/MobileLegends600x600.png',
        'FREE FIRE':
          'https://cdn1.codashop.com/S/content/common/images/mno/FreeFire600x600.png',
        'FREE FIRE MAX':
          'https://cdn1.codashop.com/S/content/common/images/mno/FreeFire600x600.png',
        'GENSHIN IMPACT':
          'https://cdn1.codashop.com/S/content/common/images/mno/GenshinImpact600x600.png',
        'PUBG MOBILE':
          'https://cdn1.codashop.com/S/content/common/images/mno/PUBGM600x600.png',
        VALORANT:
          'https://cdn1.codashop.com/S/content/common/images/mno/Valorant600x600.png',
        'HONOR OF KINGS':
          'https://cdn1.codashop.com/S/content/common/images/mno/HOK600x600.png',
        'POINT BLANK':
          'https://cdn1.codashop.com/S/content/common/images/mno/PointBlank600x600.png',
        'COD MOBILE':
          'https://cdn1.codashop.com/S/content/common/images/mno/Codm600x600.png',
        'STEAM WALLET (IDR)':
          'https://cdn1.codashop.com/S/content/common/images/mno/Steam600x600.png',
      };
      ...
      for (const item of gameItems) {
        ...
        const sortOrder = isPopular
          ? 100 - popularBrands.indexOf(brandUpper)
          : 0;
        const thumbnail =
          gameThumbnails[brandUpper] ||
          `https://www.google.com/s2/favicons?sz=128&domain=${catSlug}.com`;
  ...
  async syncSupplierBalance() {
    ...
    try {
      ...
      if (currentBalance < 200000) {
        // Turunkan limit alert ke 200rb
        this.logger.warn(
          `LOW BALANCE ALERT: Rp ${currentBalance.toLocaleString('id-ID')}.`,
        );
      }
    } catch (err: any) {
