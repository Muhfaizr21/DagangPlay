...
export class WhatsappService {
  ...
  private readonly apiUrl =
    process.env.WHATSAPP_URL || 'https://api.fonnte.com/send';
  ...
  async sendMessage(target: string, message: string) {
    ...
    try {
      ...
      console.log(
        `[Whatsapp] API Success for ${normalizedTarget}:`,
        response.data,
      );
      return response.data;
    } catch (error: any) {
      console.error(
        `[Whatsapp] API Error for ${normalizedTarget}:`,
  ...
  async sendFulfillmentNotification(
    ...
    let msg =
      `${statusEmoji} *UPDATE PESANAN - DAGANGPLAY*\n\n` +
      `Order ID: *${orderNumber}*\n` +
      `Produk: ${productName}\n` +
      `Status: *${status}*\n`;
