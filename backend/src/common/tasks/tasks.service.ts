...
export class TasksService {
  ...
  async handleFrequentChecks() {
    ...
    try {
      ...
      if (expiredPlans.count > 0)
        this.logger.log(
          `Downgraded ${expiredPlans.count} merchants to FREE plan (after 3-day grace period).`,
        );
      ...
      for (const comm of pendingCommissions) {
        await this.prisma.$transaction(async (tx) => {
          ...
          const check = await tx.commission.findUnique({
      ...
      if (pendingCommissions.length > 0)
        this.logger.log(
          `Auto-settled ${pendingCommissions.length} orphaned commissions.`,
        );
    } catch (error) {
  ...
  async monitorFails() {
    ...
    try {
      ...
      if (failedCount >= 3) {
        this.logger.error(
          `[ALERT] Detected ${failedCount} fulfillment failures in the last 15 minutes!`,
        );
