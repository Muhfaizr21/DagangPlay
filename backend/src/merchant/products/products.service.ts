...
export class ProductsService {
  ...
  async getProducts(merchantId: string, search?: string, categoryId?: string) {
    ...
    return products.map((product) => {
      const mappedSkus = product.skus.map((sku) => {
        const merchantPriceDetails =
          sku.merchantProductPrices.length > 0
            ? sku.merchantProductPrices[0]
            : null;
        ...
        const finalPrice = merchantPriceDetails
          ? Number(merchantPriceDetails.customPrice)
          : defaultTierPrice;
        const isActive = merchantPriceDetails
          ? merchantPriceDetails.isActive
          : true; // Default to true so they are visibly ON
  ...
  async updateSkuPriceOverrides(
    ...
    if (customPrice < merchantModalPrice) {
      throw new BadRequestException(
        `Harga jual (Rp ${customPrice.toLocaleString('id-ID')}) tidak boleh lebih rendah dari harga modal (Rp ${merchantModalPrice.toLocaleString('id-ID')}) untuk Plan ${merchant?.plan}.`,
      );
    }

    // Enforce SaaS Limit
    if (isActive) {
      await this.subscriptionsService.checkFeatureLimit(
        merchantId,
        'maxProducts',
      );
    }
  ...
  async bulkUpdateMargin(
    ...
    const existingPrices = await this.prisma.merchantProductPrice.count({
      where: {
        merchantId,
        productSkuId: { in: skus.map((s) => s.id) },
        isActive: true,
      },
    });
    ...
    await this.subscriptionsService.checkFeatureLimit(
      merchantId,
      'maxProducts',
      newAdditionsCount > 0 ? newAdditionsCount : 0,
    );
    ...
    const operations = skus.map((sku) => {
      ...
      const newPrice =
        defaultPrice + defaultPrice * (markupPercentage / 100) + markupAmount;

      return this.prisma.merchantProductPrice.upsert({
        where: {
          merchantId_productSkuId: { merchantId, productSkuId: sku.id },
        },
