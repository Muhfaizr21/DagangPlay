...
export class ProductsController {
  ...
  async updateSkuPrice(
    ...
    @Body() body: { sellingPrice: number; isActive: boolean },
  ) {
    const merchant = await this.prisma.merchant.findUnique({
      where: { ownerId: req.user.id },
      select: { id: true },
    ...
    if (!merchant) throw new Error('Merchant not found');
    return this.productsService.updateSkuPriceOverrides(
      merchant.id,
      req.user.id,
      skuId,
      body.sellingPrice,
  ...
  async bulkUpdatePricing(
    ...
    if (!merchant) throw new Error('Merchant not found');
    return this.productsService.bulkUpdateMargin(
      merchant.id,
      req.user.id,
  ...
  async updateProductMetadata(
    ...
    if (!merchant) throw new Error('Merchant not found');
    return this.productsService.updateProductOverride(
      merchant.id,
      productId,
      body,
    );
