...
export class DashboardService {
  ...
  async getDashboardData(userId: string) {
    ...
    const recentOrders = await this.prisma.order.findMany({
      ...
      include: {
        ...
        productSku: { select: { product: { select: { name: true } } } },
      },
    });
    ...
    if (merchant.plan === 'FREE' && revenueMonth > 5000000)
      alerts.push(
        'Omset Anda sudah mencapai limit Paket FREE. Silakan upgrade!',
      );
  ...
  async getDashboardReport(userId: string) {
    ...
    csv +=
      'ID Order,Customer,Produk,SKU,Total,Status Bayar,Status Fulfillment,Waktu\n';
