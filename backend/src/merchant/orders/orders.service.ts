...
export class OrdersService {
  ...
  async createDirectOrder(
    ...
    if (merchant.availableBalance < modalPrice) {
      throw new BadRequestException(
        'Saldo Toko Anda tidak mencukupi untuk pesanan direct ini. Silakan top-up terlebih dahulu.',
      );
    }
    ...
    return this.prisma
      .$transaction(async (tx) => {
        ...
        if (!updatedMerchant) {
          throw new BadRequestException(
            'Saldo Toko tidak mencukupi atau sedang dikunci sistem.',
          );
        }
      ...
      .then(async (order) => {
        ...
        console.log(
          `[DirectOrder] Enqueued fulfillment for order: ${order.orderNumber}`,
        );
        return order;
      });
  }
  ...
  async getOrders(merchantId: string, filters: any) {
    ...
    const successCount = await this.prisma.order.count({
      where: { merchantId, fulfillmentStatus: 'SUCCESS' },
    });
      ...
      include: {
        ...
        productSku: {
          select: {
            name: true,
            product: {
              select: { name: true, categoryId: true, thumbnail: true },
            },
          },
        },
        statusHistories: {
  ...
  async retryOrder(merchantId: string, orderId: string) {
    ...
    if (updatedCount.count === 0) {
      throw new BadRequestException(
        'Order tidak bisa diretry. Pastikan order sudah terbayar dan tidak sedang/sudah diproses (SUCCESS/PROCESSING).',
      );
    }
    ...
    return {
      message: 'Order retry triggered via persistent BullMQ queue',
      order: updated,
    };
  }
  ...
  async refundOrder(merchantId: string, orderId: string, reason: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        `Order cannot be refunded (status: ${order.fulfillmentStatus}, payment: ${order.paymentStatus})`,
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (orderCheck.count === 0) {
        throw new BadRequestException(
          'Order tidak dapat direfund (Mungkin sudah Sukses atau sudah Direfund sebelumnya)',
        );
      }
      ...
      if (isDirectOrder) {
        ...
        // 2. CLAW BACK COMMISSIONS (Merchant & Platform)
        ...
        for (const commission of commissions) {
          ...
          if (commission.status === 'PENDING') {
            // Merchant Escrow Clawback
            const merchant = await tx.merchant.findUnique({
              where: { ownerId: beneficiary.id },
            });
            if (merchant) {
              const updatedMerchant = await tx.merchant.update({
            ...
            // Settled Funds Clawback (Merchant Available or Admin Balance)
            if (beneficiary.role === 'MERCHANT') {
              ...
              beneficiary.role === 'SUPER_ADMIN' ||
              beneficiary.role === 'ADMIN_STAFF'
            ) {
