...
export class CommissionsController {
    constructor(private readonly commissionsService: CommissionsService, private prisma: PrismaService) { }
    ...
    async settleResellerCommissions(@Request() req, @Param('resellerId') resellerId: string) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id } });
        if (!merchant) throw new Error('Merchant not found');
