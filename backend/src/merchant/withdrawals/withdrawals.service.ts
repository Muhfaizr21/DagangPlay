...
export class WithdrawalsService {
  ...
  async requestWithdrawal(
    ...
    if (!merchant)
      throw new BadRequestException('Merchant data not found for this user');
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!currentMerchant || currentMerchant.availableBalance < amount) {
        throw new BadRequestException(
          'Saldo Toko tidak mencukupi untuk penarikan ini.',
        );
      }
  ...
  async approveWithdrawal(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (updateResult.count === 0) {
        throw new BadRequestException(
          'Permintaan penarikan tidak ditemukan, diproses ganda, atau bukan PENDING.',
        );
      }
  ...
  async rejectWithdrawal(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const merchant = await tx.merchant.findUnique({
        where: { ownerId: withdrawal.userId },
      });
