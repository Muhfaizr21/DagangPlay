...
export class FinanceService {
    ...
    const totalProfit = orders.reduce(
      (sum, order) =>
        ...
        (Number(order.totalPrice) -
          Number(order.merchantModalPrice || order.totalPrice)),
      0,
    );
  ...
  async requestWithdrawal(
    ...
    if (netAmount <= 0)
      throw new BadRequestException(
        'Amount tidak mencukupi setelah dikurangi fee',
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!merchant || merchant.ownerId !== ownerId) {
        throw new BadRequestException(
          'Anda tidak berwenang melakukan penarikan untuk toko ini',
        );
      }

      if (merchant.availableBalance < amount) {
        throw new BadRequestException(
          'Saldo tersedia tidak mencukupi untuk penarikan ini',
        );
      }
      ...
      return tx.withdrawal.create({
        data: {
          ...
          note: isInstant
            ? 'Diterima sebagai penarikan INSTAN (Sedang diproses sistem payout/admin)'
            : undefined,
        },
      });
    });
  ...
  async requestDeposit(
    ...
    const methodMapping: Record<
      string,
      { tripay: string; prisma: PaymentMethod }
    > = {
