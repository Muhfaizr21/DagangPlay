import { Controller, Get, Body, Param, UseGuards, Request, Post, Query, Patch, Put } from '@nestjs/common';
...
export class MembersController {
    ...
    @Get()
    async getMembers(@Request() req, @Query('search') search?: string, @Query('role') role?: Role) {
    ...
    @Post()
    async createMember(@Request() req, @Body() data: { name: string, phone: string, balance?: number }) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id }, select: { id: true } });
        if (!merchant) throw new Error('Merchant not found');
    ...
    @Put(':id')
    async updateMember(@Request() req, @Param('id') userId: string, @Body() data: { name: string, phone: string }) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id }, select: { id: true } });
        if (!merchant) throw new Error('Merchant not found');
    ...
    @Patch(':id/role')
    async toggleRole(@Request() req, @Param('id') userId: string, @Body('role') targetRole: string) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id }, select: { id: true } });
        if (!merchant) throw new Error('Merchant not found');
        return this.membersService.toggleResellerStatus(merchant.id, userId, targetRole as Role);
    }
