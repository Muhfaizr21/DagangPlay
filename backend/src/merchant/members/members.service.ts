...
export class MembersService {
  ...
  async createManualUser(
    merchantId: string,
    data: { name: string; phone: string; balance?: number },
  ) {
    ...
    if (existing)
      throw new BadRequestException(
        'Nomor WhatsApp sudah terdaftar sebagai member di toko Anda.',
      );
    ...
    const hashedPassword = await bcrypt.hash(
      'MEMBER_' + Math.random().toString(36).substring(7),
      10,
    );
  ...
  async updateUser(
    ...
    if (data.phone !== user.phone) {
      ...
      if (existing)
        throw new BadRequestException(
          'Nomor WhatsApp baru sudah digunakan oleh member lain.',
        );
    }
  ...
  async toggleResellerStatus(
    ...
    if (
      !([Role.CUSTOMER, Role.RESELLER] as string[]).includes(
    ...
    if (!user)
      throw new NotFoundException('User tidak ditemukan di merchant ini.');
