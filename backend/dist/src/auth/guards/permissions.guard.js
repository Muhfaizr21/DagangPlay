...
let PermissionsGuard = class PermissionsGuard {
    ...
    canActivate(context) {
        ...
        if (!user) {
            throw new common_1.ForbiddenException('Akses ditolak: User tidak teridentifikasi.');
        }
        if (user.role === client_1.Role.SUPER_ADMIN) {
            return true;
        }
        if (user.role === client_1.Role.ADMIN_STAFF) {
            const userPermissions = Array.isArray(user.adminPermissions) ? user.adminPermissions : [];
            const hasPermission = requiredPermissions.some((perm) => userPermissions.includes(perm));
            if (!hasPermission) {
                throw new common_1.ForbiddenException(`Akses ditolak: Membutuhkan izin [${requiredPermissions.join(' / ')}]`);
            }
            return true;
        }
        return true;
