...
let TeamController = class TeamController {
    teamService;
    prisma;
    constructor(teamService, prisma) {
    ...
    async updateTeamMember(req, id, body) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id } });
        if (!merchant)
            throw new Error('Merchant not found');
        return this.teamService.updateTeamMember(merchant.id, id, body);
    }
    async removeTeamMember(req, id) {
        const merchant = await this.prisma.merchant.findUnique({ where: { ownerId: req.user.id } });
        if (!merchant)
            throw new Error('Merchant not found');
...
exports.TeamController = TeamController = __decorate([
    ...
    (0, common_1.Controller)('merchant/team'),
    __metadata("design:paramtypes", [team_service_1.TeamService, prisma_service_1.PrismaService])
], TeamController);
