...
export default function AdminPricingPage() {
    const { data: skus, error, isLoading } = useSWR('http://localhost:3001/admin/products/skus/pricing', fetcher);
    ...
    const handleSave = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            await axios.patch(`http://localhost:3001/admin/products/skus/${id}/price`, editValues, {
                headers: { Authorization: `Bearer ${token}` }
            });
            mutate('http://localhost:3001/admin/products/skus/pricing');
            setEditingId(null);
        ...
        <AdminLayout>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 flex flex-col md:flex-row gap-4 items-center justify-between transition-all">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <button
                        onClick={() => mutate('http://localhost:3001/admin/products/skus/pricing')}
                        disabled={isLoading}
