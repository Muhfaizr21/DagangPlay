...
export default function MarketingAdmin() {
    ...
    const { data: guides, mutate } = useSWR(`${getApiUrl()}/admin/marketing/guides?search=${searchTerm}`, fetcher);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            if (editingGuide) {
                await axios.patch(`${getApiUrl()}/admin/marketing/guides/${editingGuide.id}`, form, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post((getApiUrl()) + '/admin/marketing/guides', form, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            mutate();
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('admin_token');
            await axios.delete(`${getApiUrl()}/admin/marketing/guides/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            mutate();
        } catch (err: any) {
