...
export default function PromoManagementPage() {
    ...
    const { data: promos, isLoading, mutate } = useSWR(
        `${getApiUrl()}/admin/promos?search=${searchTerm}`,
        fetcher
    );

    const { data: report } = useSWR((getApiUrl()) + '/admin/promos/report', fetcher);
    ...
    const handleToggle = async (id: string) => {
        try {
            await axios.patch(`${getApiUrl()}/admin/promos/${id}/toggle`);
            mutate();
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await axios.delete(`${getApiUrl()}/admin/promos/${id}`);
            mutate();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingPromo) {
                await axios.patch(`${getApiUrl()}/admin/promos/${editingPromo.id}`, formData);
                showToast('Update Sukses', 'Data promo telah diperbarui');
            } else {
                await axios.post((getApiUrl()) + '/admin/promos', formData);
