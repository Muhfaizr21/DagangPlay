...
export default function SecurityManagementPage() {
    ...
    // FRAUD State
    const { data: frauds, isLoading: loadingFrauds, mutate: mutateFrauds } = useSWR((getApiUrl()) + '/admin/security/fraud', fetcher);
    // BLACKLIST State
    const { data: blacklists, isLoading: loadingBlacklists, mutate: mutateBlacklists } = useSWR((getApiUrl()) + '/admin/security/blacklist', fetcher);
    const [showBlacklistModal, setShowBlacklistModal] = useState(false);
    const [blacklistForm, setBlacklistForm] = useState({ ipAddress: '', reason: '' });
    // LOGIN State
    const { data: logins, isLoading: loadingLogins } = useSWR((getApiUrl()) + '/admin/security/login-attempts', fetcher);
    // AUDIT State
    const { data: audits, isLoading: loadingAudits } = useSWR((getApiUrl()) + '/admin/security/audit', fetcher);
    ...
    const handleResolveFraud = async (id: string) => {
        ...
        try {
            await axios.post(`${getApiUrl()}/admin/security/fraud/${id}/resolve`, {}, { headers: { Authorization: `Bearer ${localStorage.getItem('admin_token')}` } });
            mutateFrauds();
    ...
    const handleAddBlacklist = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await axios.post((getApiUrl()) + '/admin/security/blacklist', blacklistForm);
            mutateBlacklists();
    ...
    const handleRemoveBlacklist = async (id: string) => {
        ...
        try {
            await axios.delete(`${getApiUrl()}/admin/security/blacklist/${id}`);
            mutateBlacklists();
